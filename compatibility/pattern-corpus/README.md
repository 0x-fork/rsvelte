...
| `special-elements/` | `svelte:options`/`head`/`window`/`document`/`body` in one component with bound and handler attributes, and `<svelte:boundary>` with an `onerror` reset around a `{#key}` whose body throws, next to its `failed` snippet | The `svelte:*` special elements as a group |
