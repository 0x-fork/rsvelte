...
## Matrix known failures (`matrix-known-failures.json`, 818 entries)

Partition of `matrix-known-failures.json` by family: `2 + 116 + 0 + 18 + 60 + 62 + 3 + 324 + 225 + 8`
...
Partition of `matrix-known-failures.json` entries under `async-derived/` by cause: `130 + 39 + 16 + 12 + 13 + 13 + 2`

| # | cause | entries |
|---|---|---|
| 1 | `<script module>` / `compileModule` async-derived lowering | 130 |
| 2 | the `$$d` temp appears in the hoisted `var` list | 39 |
| 3 | `svelte-ignore` comment not reproduced on the hoisted declaration | 16 |
