...
**つまり残りは単に未検証なのではなく、★ 過大側に偏っていると仮定すること。**
M3/M4 の µs を引用するときは「過大の可能性が高い」を必ず添える。
（M1 と M2 の実測がどちらも 6.4µs になったのは**偶然**。出典を添えて書き分けること。）
...
#### ★ M2 の「−20〜25µs」は実測上限 6.4µs（2026-08-06 計測）★
...
```
★ 見出し  78.3 entries/file x 81.5 ns = 6.4 µs/file
          （傾き由来。傾きの median/min = 1.11 で汚染ゲート通過）
