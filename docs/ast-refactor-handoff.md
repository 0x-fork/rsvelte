...
| M2 | テンプレート式変換の単一 AST パス化（`has_reactive_state_json` 550 行の typed 化を吸収） | ~~−20〜25µs~~ → **実測上限 6.4〜8.0µs/file**（下記） | 220 件回帰の再演を検知したら即 revert |
...
**検算した 2 件が 2 件とも、同じ方向（過大）に外れています**: M1 が **5 倍**、M2 が **3〜4 倍**。
**M3〜M4 は同じ人が同じ方法で出した数字**です。**「たまたま外れた」と読まないこと。**
金額は全部検算されていないものとして扱い、各 M に入る前に決定論カウンタで検算する。
...
#### ★ M2 の「−20〜25µs」は実測上限 6.4〜8.0µs（2026-08-06 計測）★

M2 が消すのは `TypedExpr::as_json()` が作る `serde_json::Value`。**カウンタも計時も既存**:

```
cargo run --release -p rsvelte_devtools --bin json_materialize_count \
  --features measure-json [--mode client|server]
```

**除去可能な量**（flowbite 1,296 files。`--mode server` との差で client 固有分を切り出した）:

```
                  client    server    差（= client 固有）
materializations   7,073       996     6,077  (86%)
map entries      123,559    21,998   101,561   →  78.3 /file
```

`as_json()` は 31 ファイル・135 箇所から呼ばれ、phase 1 に 21・phase 2 に ~12 ある。
キャッシュは per-`TypedExpr` の `OnceCell` なので**最初に呼んだ者だけが払う**。
「先に parse/analyze が触るので phase 3 を直しても減らない」を疑ったが、
**server 差で 86% が client 固有と分かった。この懸念は誤り。**

**コスト定数**（`as_json` の `to_value()` を実際の呼び出し位置で計時、8 run）:

```
                       min    median    max     median/min
合計 µs/file          7.51     9.8    48.59      1.30  ← 汚染ゲート 1.2 に落ちる
per-entry 傾き (ns)  73.46    81.5   187.45      1.11  ← 通る。★ こちらを使う
per-call 切片 (ns)   -846      ~130   1382       ★ 符号が反転。同定できていない
```

**切片でなく傾きを使う理由は 2 つ**: (1) 切片は 8 run 中 2 本で負になり同定できていない、
(2) **計時の時計 2 回分は per-call 側に全部乗るので、傾きは計器を測っていない半分**。

```
経路 1（傾き）  78.3 entries/file x 81.5 ns = 6.4 µs/file
経路 2（総和）  9.8 µs/file x 82%（client 固有の entry シェア）= 8.0 µs/file
→ 6.4〜8.0 µs/file = flowbite の 1 ファイル平均 491.81µs の 1.3〜1.6%
```
...
#### ★ M1 の「−30〜35µs」は実測で裏付けが取れない（2026-08-06 計測）★

M1 が消すのは意味論ではなく配管（パス毎の「パース → 編集収集 → テキストに戻す」往復）なので、
**その配管そのものを数えた**。カウンタなので負荷非依存。
再現: `RSVELTE_AST_DUAL_RUN=1 dual_run_tally <dir>`（dev=false/true の 2 回コンパイル）。

```
corpus          files  compiles   parses  /comp   parsedB  B/comp  splices  movedB  B/comp
flowbite         1296      2592     1111   0.43    841841     325       86  113215      44
shadcn-svelte    1682      3364     1085   0.32    427397     127       18   31168       9
layerchart       1400      2800     1732   0.62    784949     280      252   33744      12
bits-ui           617      1234      517   0.42    287955     233        1     922       1
skeleton          686      1372      614   0.45    296729     216       21   22846      17
svelte-ux         198       396     4011  10.13   1258800    3179      231  610512    1542
svelte fixtures  4459      8918    10885   1.22   1090226     122     1054  221819      25
```

**現代の実プロジェクト 5 本では往復が 1 コンパイルあたり 0.32〜0.62 回しか起きない。**
「37 本のパスが毎回往復する」は、レガシー構文が濃い svelte-ux（10.13 回）にしか当てはまらない。
これは「現代の大規模プロジェクトは `$:` ゼロ」というレガシー密度の実測と整合する。

時間側（`compile_profile --shipped`、n=5879）:

| 項目 | compile 比 |
|---|---|
| driver の parse + visit（印字込み） | 0.54% |
| direct parse（10 箇所、印字しない） | 0.89% |
| splice のバイト移動 44 B/compile | memcpy 換算で事実上 0 |
| **M1 が消せる配管の合計** | **~1.4% ≈ 6.4µs/file** |

**したがって上表の「−30〜35µs」は 5 倍の過大。** ただしこれは M1 の否定ではない:
in-place 化が副次的に持ち込む効果（1 回パース化、順序固定）は測っていないし、
