...
fn run(source: &str) -> String {
    ...
    if let Some(module) = ast.module.as_mut() {
        phase1_parse::read::script::ensure_script_parsed(&ast.arena, module, source, &line_offsets);
    }
    ...
    let analysis = phase2_analyze::analyze_component(&mut ast, source, &options).expect("analyze");
...
fn run_async_both(source: &str) -> (String, String) {
    ...
    let ours =
        server_component_ast(&analysis, &ast, source, &options, &allocator).expect("ast output");
    let oracle = super::super::transform_server(&analysis, &ast, source, &options).expect("server");
    (ours, oracle)
}
...
fn ast_matches_oracle_component_children() {
    ...
    for (label, body) in cases {
        ...
        let collapse = |v: &[String]| v.join(" ").split_whitespace().collect::<Vec<_>>().join(" ");
        ...
        .spawn(|| {
            ...
            for name in names {
                ...
                for path in entries {
                    ...
                    let label = format!("{name}/{}", path.file_name().unwrap().to_str().unwrap());
                    assert_eq!(
                        canon_js(&ours),
                        canon_js(&oracle),
...
fn ast_matches_oracle_component_bindings_settle_loop() {
    ...
    assert!(
        ours.contains("$$render_inner($$inner_renderer)") && ours.contains("} while (!$$settled);"),
...
fn ast_matches_oracle_class_style_directives() {
    ...
    let ours = run("<script>let a = $state(true);</script><div class={'y'} class:foo={a}></div>");
    assert!(
...
fn compile_both(source: &str) -> Outcome {
    ...
    let (matched_struct, used_fallback, new_canon, oracle_canon) = match (new_canon, oracle_canon) {
...
fn ast_matches_oracle_derived_readwrap_cluster() {
    let manifest = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let base = manifest.join("../../submodules/svelte/packages/svelte/tests/runtime-runes/samples");
    let names = [
        "derived-stale-value",
